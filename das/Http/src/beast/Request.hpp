...
public:
    ...
    const nlohmann::json& JsonBody() const noexcept
    {
        return json_body_;
    }
    
    const request_type& RawRequest() const noexcept
    {
        return request_;
    }
...
public:
    ...
    HttpResponse(http::status status = http::status::ok, unsigned int version = 11)
    ...
    response_type&& Release() noexcept
    {
        return std::move(response_);
    }
    
    static HttpResponse CreateErrorResponse(DasResult error_code, const std::string& message)
    {
    ...
    static HttpResponse CreateSuccessResponse(const nlohmann::json& data = nullptr)
    {
        HttpResponse response;
        nlohmann::json body;
        body["code"] = DAS_S_OK;
