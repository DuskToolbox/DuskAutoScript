...
namespace
{
    ...
    RepositoryEntryDto MakeRepositoryEntry(
        int64_t entry_id,
        ...
        const yyjson::value& compiled_artifact = yyjson::value{})
    {
        ...
        entry.graph_document = yyjson::to_json(graph_doc);
        entry.compiled_artifact = compiled_artifact;
        return entry;
    }

    EntryAccessor MockEntryAccessor(
        const std::map<GraphEntryId, RepositoryEntryDto>& entries)
    {
        return [entries](GraphEntryId id) -> std::optional<RepositoryEntryDto>
        {
...
TEST(SubgraphCompilerTest, CompileEntryRefSingleSubgraph)
{
    ...
    auto outer_doc = MakeGraphDocument(
        {MakeComponentNode("A", "guid-A"),
         MakeEntryRefNode("B", 100)});
...
TEST(SubgraphCompilerTest, CompileEntryRefProducesCompiledPlan)
{
    ...
    // binding_plan should have bindings for all inner edges
    ASSERT_EQ(snapshot.compiled_plan.binding_plan.bindings.size(), 1u);
    EXPECT_EQ(
        snapshot.compiled_plan.binding_plan.bindings[0].source_node_id, "X");
}
...
TEST(SubgraphCompilerTest, CompileRecursiveMultiLevel)
{
    ...
    auto level2_doc = MakeGraphDocument({MakeComponentNode("L2_A", "g-L2")});
    ...
    auto accessor =
        MockEntryAccessor({{200, entry_200}, {300, entry_300}});
    ...
    EXPECT_EQ(snapshot.nested_snapshots[0].nested_snapshots[0].entry_id, 300);
}
...
TEST(SubgraphCompilerTest, CompileRecursiveCycleDetection)
{
    ...
    auto accessor =
        MockEntryAccessor({{100, entry_100}, {200, entry_200}});
    ...
    // At least one diagnostic about cycle detection
    bool has_cycle_diagnostic = false;
    for (const auto& ns : snapshot.nested_snapshots)
    {
        for (const auto& diag : ns.diagnostics)
        {
...
TEST(SubgraphCompilerTest, CompileRecursiveSelfReference)
{
    ...
    // Should detect self-reference and produce diagnostic
...
TEST(SubgraphCompilerTest, CompileSubgraphWithNoEntryRefs)
{
    ...
    auto doc = MakeGraphDocument(
        {MakeComponentNode("A", "g-A"),
         MakeComponentNode("B", "g-B")},
...
TEST(SubgraphCompilerTest, EntryAccessorCallbackCalled)
{
    ...
    EntryAccessor tracking_accessor =
        [&called_ids,
         entry_100](GraphEntryId id) -> std::optional<RepositoryEntryDto>
    {
...
TEST(SubgraphCompilerTest, NestedSnapshotPreservesParentEntryId)
{
    ...
    auto inner_doc_l1 = MakeGraphDocument(
        {MakeComponentNode("Y", "g-Y"),
         MakeEntryRefNode("Y_ref", 300)});

    auto outer_doc = MakeGraphDocument(
        {MakeEntryRefNode("ref", 200)});
