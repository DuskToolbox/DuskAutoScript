...
DasResult CvCudaImpl::TemplateMatchBest(
    ...
    ExportInterface::IDasTemplateMatchResult** pp_out_result)
{
    ...
    DAS_CORE_LOG_INFO(
        "Function cuda::matchTemplate and minMaxLoc cost {} ms.",
        cv_cost);
...
DasResult CvCudaImpl::TemplateMatchAll(
    ExportInterface::IDasImage*                 p_image,
    ExportInterface::IDasImage*                 p_template,
    ExportInterface::DasTemplateMatchType       type,
    double                                      threshold,
    ...
    ExportInterface::IDasTemplateMatchResults** pp_out_results)
{
    ...
    // Step 5: Create result collection
    auto* p_results =
...
DasResult CvCudaImpl::CreateMatchConfig(
...
DasResult CvCudaImpl::MatchFeatures(
    ...
    ExportInterface::IDasCvMatchResult** pp_out_result)
{
    ...
    // Fallback to CPU for feature matching
    ...
    DAS_CORE_LOG_INFO(
        "Feature matching completed in {} ms, {} matches (CPU fallback)",
        elapsed,
        matches.size());
...
DasResult CvCudaImpl::ConvertColor(
    ...
    ExportInterface::IDasImage**         pp_out_image)
{
    ...
    cv::cuda::GpuMat& gpu_src = backend.GetGpuMat();
...
DasResult CvCudaImpl::ColorFilter(
    ExportInterface::IDasImage*           p_src,
