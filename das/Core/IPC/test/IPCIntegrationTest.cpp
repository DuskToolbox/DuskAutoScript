/**
 * @file IPCIntegrationTest.cpp
 * @brief IPC 多进程集成测试
 *
 * 测试主进程 ↔ Host 进程通信、RemoteObjectRegistry 注册/查找、Proxy 调用。
 *
 * 架构说明：
 * - 主进程 (测试进程)：负责启动 Host 进程、通过 IPC 调用远程接口
...
TEST_F(IPCIntegrationTest, SessionEstablishAndDisconnect)
{
    // 初始化 Host 进程
...
TEST_F(IPCIntegrationTest, MultipleHostConnections)
{
    ...
    EXPECT_EQ(connected_sessions.size(), 3);
    ...
    connected_sessions =
        MainProcessServer::GetInstance().GetConnectedSessions();
    EXPECT_EQ(connected_sessions.size(), 0);
}

// ====== 远程对象注册与查找测试 ======

TEST_F(IPCIntegrationTest, RemoteObjectRegisterAndLookup)
{
    // 初始化 Host 进程并连接
    ASSERT_EQ(host_process_.Initialize(), DAS_S_OK);
    uint16_t host_session_id = host_process_.GetSessionId();
...
TEST_F(IPCIntegrationTest, MultipleRemoteObjectsFromSameHost)
{
    // 初始化 Host 进程并连接
    ASSERT_EQ(host_process_.Initialize(), DAS_S_OK);
    uint16_t host_session_id = host_process_.GetSessionId();
...
TEST_F(IPCIntegrationTest, RemoteObjectUnregistration)
{
    // 初始化 Host 进程并连接
    ASSERT_EQ(host_process_.Initialize(), DAS_S_OK);
    uint16_t host_session_id = host_process_.GetSessionId();
...
TEST_F(IPCIntegrationTest, AutoCleanupOnSessionDisconnect)
{
    // 初始化 Host 进程并连接
    ASSERT_EQ(host_process_.Initialize(), DAS_S_OK);
    uint16_t host_session_id = host_process_.GetSessionId();
...
TEST_F(IPCIntegrationTest, DISABLED_ProxyCreationAndBasicUsage)
{
    ...
    ObjectId    obj_id = {host_session_id, 1, 100};
    DasGuid     iid = CreateTestGuid(0x12345678);
    std::string obj_name = "TestProxyObject";
    ...
    // 创建 Proxy
    auto proxy = factory.GetProxy(obj_id);
    // 注意：实际 Proxy 创建需要 IpcRunLoop，这里只验证基础功能
...
TEST_F(IPCIntegrationTest, LookupNonExistentObject)
{
    RemoteObjectInfo info;
    EXPECT_EQ(
...
TEST_F(IPCIntegrationTest, InvalidSessionOperations)
{
...
TEST_F(IPCIntegrationTest, ConcurrentSessionAllocation)
{
...
TEST_F(IPCIntegrationTest, MessageDispatch)
{
    // 初始化 Host 进程并连接
    ASSERT_EQ(host_process_.Initialize(), DAS_S_OK);
    uint16_t host_session_id = host_process_.GetSessionId();
...
TEST_F(IPCIntegrationTest, SessionEventCallbacks)
{
...
TEST_F(IPCIntegrationTest, ObjectEventCallbacks)
{
...
TEST_F(IPCIntegrationTest, Performance_ObjectRegistration)
{
    // 初始化 Host 进程并连接
    ASSERT_EQ(host_process_.Initialize(), DAS_S_OK);
    uint16_t host_session_id = host_process_.GetSessionId();
    ...
    std::vector<RemoteObjectInfo> objects;
    MainProcessServer::GetInstance().GetRemoteObjects(objects);
    EXPECT_EQ(objects.size(), num_objects);
}
