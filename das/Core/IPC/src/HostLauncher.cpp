...
namespace Core
{
    namespace IPC
    {
        ...
        uint16_t HostLauncher::GetSessionId() const
        {
            return impl_->session_id;
        }

        IpcTransport* HostLauncher::GetTransport()
        {
        ...
        DasResult HostLauncher::ConnectToHost()
        {
            ...
            DasResult result = impl_->transport->Connect(
                host_to_plugin_queue,
                plugin_to_host_queue);
        ...
        DasResult HostLauncher::SendHandshakeHello(
            const std::string& client_name)
        {
            ...
            header.interface_id = static_cast<uint32_t>(HandshakeInterfaceId::HANDSHAKE_IFACE_HELLO);
            header.method_id = 0;
            header.flags = 0;
            header.error_code = 0;
            ...
            uint32_t  timeout_ms)
        {
            ...
            DasResult result =
                impl_->transport->Receive(header, body, timeout_ms);
            if (result != DAS_S_OK)
            {
                std::string msg = DAS_FMT_NS::format(
        ...
        DasResult HostLauncher::SendHandshakeReady(uint16_t session_id)
        {
            ...
            header.header_flags = 0;
            header.call_id = impl_->next_call_id++;
            header.interface_id = static_cast<uint32_t>(HandshakeInterfaceId::HANDSHAKE_IFACE_READY);
            header.method_id = 0;
            header.flags = 0;
            header.error_code = 0;
