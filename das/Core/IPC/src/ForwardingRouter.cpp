...
bool ForwardingRouter::AddRoute(const RouteKey& key, const RouteTarget& target)
{
...
size_t ForwardingRouter::GetRouteCount() const { return route_table_.size(); }
...
bool ForwardingRouter::FindTarget(const RouteKey& key, RouteTarget& out_target)
    const
{
    auto it = route_table_.find(key);
    if (it != route_table_.end())
