...
DasResult RemoteObjectRegistry::UnregisterObject(const ObjectId& object_id)
{
    if (IsNullObjectId(object_id))
    {
        return DAS_E_IPC_INVALID_OBJECT_ID;
    ...
    if (name_it != objects_by_name_.end() && name_it->second == encoded_id)
    {
        objects_by_name_.erase(name_it);
    }

    // 从接口索引移除
    auto interface_it = objects_by_interface_.find(entry.info.interface_id);
