...
"""
    ...
    def _generate_deserialize_param(self, param: ParameterDef, indent: str, has_return: bool = True, method: MethodDef = None) -> List[str]:
        ...
        if is_interface and param.type_info.is_pointer:
            ...
            lines.append(f"{indent}ipc_result = Das::Core::IPC::DeserializeInInterfaceParam(")
            lines.append(f"{indent}    Das::Core::IPC::EncodeObjectId({pn}_oid),")
            lines.append(f"{indent}    {pn}_interface_id,")
            lines.append(f"{indent}    GetObjectManager(),")
            lines.append(f"{indent}    GetRunLoop(),")
            lines.append(f"{indent}    GetBusinessThread(),")
            lines.append(f"{indent}    GetProxyFactory(),")
            lines.append(f"{indent}    reinterpret_cast<IDasBase**>({pn}));")
            lines.append(f"{indent}if (DAS::IsFailed(ipc_result)) {{ return ipc_result; }}")
            return lines
