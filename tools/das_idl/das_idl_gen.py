...
def generate_type_maps_from_jsons(json_files: List[str], output_path: str, build_dir: Optional[Path] = None) -> None:
    ...
    try:
            ...
            try:
                ...
                if 'typemaps_ignore' in data:
                    for sig, info in data['typemaps_ignore'].items():
                        # 去 :: 前缀后去重，避免同一方法出现两个 %ignore
                        # （如 SetBase(::IDasReadOnlyString*, ...) 和 SetBase(IDasReadOnlyString*, ...)）
                        normalized_sig = sig.replace('::', '')
                        if normalized_sig not in ignore_typemaps:
                            ignore_typemaps[normalized_sig] = info
                ...
                if 'director_typemaps' in data:
                    for typ, info in data['director_typemaps'].items():
                        if typ not in all_director_typemaps:  # Use first occurrence (deduplicate)
                            all_director_typemaps[typ] = info
            except Exception as e:
        ...
        # Generate DasDirectorTypemaps.i — type-level director out typemaps aggregated here
        # to prevent SWIG 4.4.1 cross-file feature table pollution
        director_lines = []
        director_lines.append("// DasDirectorTypemaps.i - Director out param type-level typemaps")
        director_lines.append("// This file is automatically generated by das_idl_gen.py")
        director_lines.append("// !!! DO NOT EDIT !!!")
        director_lines.append("// Aggregates type-level typemaps (jni/jtype/jstype/in/javain/directorin/directorout)")
        director_lines.append("// to prevent SWIG 4.4.1 cross-file feature table pollution when included via swig_all.i\n")
        for typ in sorted(all_director_typemaps.keys()):
            director_lines.append(f"// {typ}")
            director_lines.append(all_director_typemaps[typ]['code'] if isinstance(all_director_typemaps[typ], dict) else all_director_typemaps[typ])
            director_lines.append("")
        director_file = output_dir / "DasDirectorTypemaps.i"
        director_file.parent.mkdir(parents=True, exist_ok=True)
        director_content = "\n".join(director_lines)
        if _write_if_changed(director_file, director_content):
            print(f"Generated DasDirectorTypemaps.i: {director_file}")
