...
"""
    ...
    def _generate_handle_method_definition(self, interface: InterfaceDef, method: MethodDef, method_index: int) -> str:
        ...
        for param in in_params:
            ...
            if param.type_info.base_type == "IDasReadOnlyString":
                continue
            # 使用 type_kind 检测接口类型
            is_interface = (param.type_info.type_kind == TypeKind.INTERFACE and param.type_info.is_pointer)
            if not is_interface:
                continue
    ...
    def _generate_deserialize_param_for_stub(self, param: ParameterDef, indent: str) -> List[str]:
        ...
        is_interface_for_deser = (param.type_info.type_kind == TypeKind.INTERFACE and param.type_info.is_pointer)
